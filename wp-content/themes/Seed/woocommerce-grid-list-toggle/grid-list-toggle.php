	...
	if (!class_exists('WC_List_Grid')) {
		...
		class WC_List_Grid {
			...
			function setup_scripts_styles() {
				if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
					wp_enqueue_style( 'grid-list-layout', ORI_BASE_URL . '/woocommerce-grid-list-toggle/assets/css/style.css', __FILE__ );
					wp_enqueue_style( 'grid-list-button', ORI_BASE_URL . '/woocommerce-grid-list-toggle/assets/css/button.css', __FILE__ );
				}
			}
			function setup_scripts_script() {
				if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
					wp_enqueue_script( 'cookie', ORI_BASE_URL . '/woocommerce-grid-list-toggle//assets/js/jquery.cookie.min.js', __FILE__ , array( 'jquery' ) );
					wp_enqueue_script( 'grid-list-scripts', ORI_BASE_URL . '/woocommerce-grid-list-toggle//assets/js/jquery.gridlistview.min.js', __FILE__ , array( 'jquery' ) );
			...
			function gridlist_toggle_button() {
				?>
					<nav class="gridlist-toggle">
						<a href="#" id="grid" title="<?php _e('Grid view', 'wc_list_grid_toggle'); ?>"><span><?php _e('Grid view', 'wc_list_grid_toggle'); ?></span></a>
						<a href="#" id="list" title="<?php _e('List view', 'wc_list_grid_toggle'); ?>"><span><?php _e('List view', 'wc_list_grid_toggle'); ?></span></a>
					</nav>
				<?php
			}
				...
				?>
					<script>
						if (jQuery.cookie('gridcookie') == null) {
					    	jQuery('ul.products').addClass('<?php print $default; ?>');
					    	jQuery('.gridlist-toggle #<?php print $default; ?>').addClass('active');
					    }
					</script>
				<?php
			}
