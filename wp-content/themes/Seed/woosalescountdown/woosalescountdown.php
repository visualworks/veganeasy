...
if ( !class_exists( 'woosalescountdown' ) ) {
	require_once ORI_BASE . DIRECTORY_SEPARATOR . "woosalescountdown" . DIRECTORY_SEPARATOR . "widget.php";
	//die (ORI_BASE . DIRECTORY_SEPARATOR . "woosalescountdown" . DIRECTORY_SEPARATOR . "widget.php");
}
define( 'WSCD_JS', ORI_BASE_URL . "/woosalescountdown/js/" );
...
class WSCD {
	...
	public function __construct() {
		...
		/*Setting*/
		add_action( 'after_setup_theme', array( $this, 'init_woosalecountdown' ) );
		...
		if ( $id ) {
			...
			for ( $i = 0; $i < count( $id ); $i ++ ) {
				...
				if ( trim( $product->product_type ) != 'variable' ) {
					...
					$shortcode .= '<article id="organic-deal-of-day">';
					$shortcode .= '<div class="container">';
					$shortcode .= '<div class="bg-deal-of-day product-' . $product->id . '">';
					$shortcode .= '<div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">';
					$shortcode .= '<div class="form-deal-left">';
					$shortcode .= '<div class="cover">';
					$shortcode .= '<div class="cover-images-deal">';
                    //$files = get_post_meta($product->id, 'image-deal',TRUE);
                    if (function_exists('rwmb_meta')){
                    $files = rwmb_meta( 'origanic_image-deal', 'type=file' ,$product->id);
	                  foreach ( $files as $info )
	                  {
	                  $shortcode .= '<img src="'.$info['url'].'">';
	                  }
	               	}
					$shortcode .= '</div>';
					$shortcode .= '</div>';
					$shortcode .= '</div>';
					$shortcode .= '</div>';
					$shortcode .= '<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">';
					$shortcode .= '<div class="form-deal-right">';
					$shortcode .= '<div class="title-deal">//deal of the days</div>';
					...
					$ct =  get_object_vars($product);
					$content = get_object_vars($ct["post"]);
					$shortcode .= '<div class="description-product">' . $content["post_content"]. '</div>';

					if ( trim( $ob_product_show_price ) == 'yes' ) {
						$shortcode .= '<span class="price">' . strip_tags($product->get_price_html(),'<del><span>') . '</span>';
					}
					/*Get time schedule*/
	...
	function obScriptInit() {
		wp_enqueue_script( 'wscd-script-name', ORI_BASE_URL .'/'. basename( dirname( __FILE__ ) ) . '/js/jquery.mb-comingsoon.min.js', array(), false, true );
		wp_enqueue_style( 'wscd-style-name', ORI_BASE_URL .'/'.  basename( dirname( __FILE__ ) ) . '/css/woosalescountdown.css' );
	}
