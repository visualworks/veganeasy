...
<div class="row row-large <?php if(flatsome_option('blog_layout_divider')) echo 'row-divided ';?>">
	...
	<?php
		if(is_single()){
			get_template_part( 'template-parts/posts/single-receita');
			comments_template();
		} elseif(flatsome_option('blog_style_archive') && (is_archive() || is_search())){
			get_template_part( 'template-parts/posts/archive-receita', flatsome_option('blog_style_archive') );
		} else {
			get_template_part( 'template-parts/posts/archive-receita', flatsome_option('blog_style') );
		}
	?>
	</div> <!-- .large-9 -->
